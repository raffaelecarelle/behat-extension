...
use Algoritma\BehatExtension\Element\Form;
use Algoritma\BehatExtension\Element\AlgoritmaPageObjectAware;
use Algoritma\BehatExtension\Element\Select;
use Algoritma\BehatExtension\Element\Select2Entities;
use App\Tests\Behat\ClearableInterface;
use App\Tests\Behat\Element\AlgoritmaForm;
use App\Tests\Behat\Element\Select2Entity;
use App\Tests\Behat\SystemConfigForm;
 ...
 */
class FormContext extends AlgoritmaFeatureContext implements AlgoritmaPageObjectAware
{
    ...
    {
        $field = $this->createAlgoritmaForm()->findField($fieldName);
        $field->focus();
    }
     ...
     */
    public function iFillFormWith(TableNode $table, $formName = "AlgoritmaForm")
    {
        /** @var Form $form */
//        $this->waitForAjax();
    ...
    public function iOpenSelectEntityPopup($fieldName, $formName = "AlgoritmaForm")
    {
     ...
     */
    //@codingStandardsIgnoreEnd
    public function iOpenCreateEntityPopup($fieldName, $formName = "AlgoritmaForm")
    {
        /** @var Select2Entity $field */
        $field = $this->getFieldInForm($fieldName, $formName);
     ...
     */
    public function iClearField($fieldName, $formName = "AlgoritmaForm")
    {
        /** @var \Algoritma\BehatExtension\Context\ClearableInterface $field */
        $field = $this->getFieldInForm($fieldName, $formName);
    ...
    {
        $this->createAlgoritmaForm()->saveAndClose();
    }
    ...
    {
        $this->createAlgoritmaForm()->saveAndDuplicate();
    }
    ...
    {
        $this->createAlgoritmaForm()->saveAndCreateNew();
    }
    ...
    {
        $this->createAlgoritmaForm()->saveAndReturn();
    }
    ...
    public function setFieldWithValueAsPrimary($field, $value)
    {
        ...
        $collection = $this->createAlgoritmaForm()->findField(ucfirst($this->inflector->pluralize($field)));
    ...
    {
        $this->createAlgoritmaForm()->find('css', '.alert-error button.close')->press();
    }
     ...
     * @throws \Exception
     */
    public function iShouldSeeValidationErrors(TableNode $table, $formName = 'AlgoritmaForm')
    {
        $this->waitForValidationErrorsAssertion(function () use ($table, $formName) {
            /** @var AlgoritmaForm $form */
            $form = $this->createElement($formName);
     ...
     * @throws \Exception
     */
    public function iShouldNotSeeValidationErrors(TableNode $table, $formName = 'AlgoritmaForm')
    {
        $this->waitForValidationErrorsAssertion(function () use ($table, $formName) {
            /** @var AlgoritmaForm $form */
            $form = $this->createElement($formName);
    ...
    {
        /** @var Form $fieldSet */
        $fieldSet = $this->createAlgoritmaForm()->findField(ucfirst($this->inflector->pluralize($fieldSetLabel)));
        $fieldSet->fill($table);
    }
    ...
    {
        $this->createAlgoritmaForm()->fillField($field, $table);
    }
     ...
     */
    public function iTypeInFieldWith($locator, $value, $formName = 'AlgoritmaForm')
    {
     ...
     */
    //@codingStandardsIgnoreEnd
    public function iContinueTypingInFieldWith($locator, $value, $formName = 'AlgoritmaForm')
    {
        $locator = $this->fixStepArgument($locator);
        $value = $this->fixStepArgument($value);
        $formName = $this->fixStepArgument($formName);
    ...
    {
        $this->createAlgoritmaForm()->pressEntitySelectEntityButton($field);
    }
    ...
    {
        $button = $this->createAlgoritmaForm()->findButton($label);
        self::assertNotNull(
            $button,
    ...
    public function fieldShouldBeEnabled(string $fieldName): void
    {
        ...
        if (null === $field) {
            $field = $this->getFieldInForm($fieldName, 'AlgoritmaForm');
        }
    ...
    /**.
     * @return AlgoritmaForm
     */
    protected function createAlgoritmaForm()
    {
        return $this->createElement('AlgoritmaForm');
    }
     ...
     */
    //@codingStandardsIgnoreEnd
    public function iFillAbsoluteUrl(string $fieldName, string $url, string $formName = 'AlgoritmaForm'): void
    {
